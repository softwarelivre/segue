...
class PagSeguroPaymentFactory(PaymentFactory):
    ...
    def create(self, purchase, data=None):
        payment = super(PagSeguroPaymentFactory, self).create(purchase, target_model=self.model, extra_data=data)
...
class PagSeguroSessionFactory(object):
    ...
    def query_session(self, transaction_id):
        pg = PagSeguro(config.PAGSEGURO_EMAIL, config.PAGSEGURO_TOKEN)
        if self.use_env != 'production': pg.config = ConfigSandbox
        pg.check = lambda: pg.check_transaction(transaction_id).xml
        return pg
